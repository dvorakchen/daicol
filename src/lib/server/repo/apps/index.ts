...
import * as pgAppCreateUpdate from '$lib/server/repo/apps/create.ts';
import * as pgAppDelete from '$lib/server/repo/apps/delete.ts';
...
export const appRepoServiceId: ServiceIdentifier<AppRepo> = Symbol.for('appRepoServiceId');
...
export class PgAppRepo implements AppRepo {
	...
	updateApp(model: CreationUpdateModel): Promise<void> {
		return pgAppCreateUpdate.updateApp(model, this.bucket);
	}
	getRecommendApps(count: number = 10): Promise<AppWithoutPrompt[]> {
		return pgApp.getRecommendApps(count);
	}
	getHotApps(count: number = 10, excludeIds: number[] = []): Promise<AppWithoutPrompt[]> {
