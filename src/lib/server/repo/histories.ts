...
import { type ServiceIdentifier, injectable } from 'inversify';

export interface HistoryRepo {
  addHistory(routeId: number, accessType: AccessType): Promise<void>;
}

export const historyRepoServiceId: ServiceIdentifier<HistoryRepo> =
  Symbol.for('historyRepoServiceId');

@injectable()
export class PgHistoryRepo implements HistoryRepo {
  async addHistory(routeId: number, accessType: AccessType): Promise<void> {
